...
def employees_import(file=None):
	try:
		if not file:
			frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "file is missing", "user": frappe.session.user})
		...
		if len(excel_data_df) > 0:
			...
			if len(get_marsha_details) == 0:
				frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "no marsha details exists", "user": frappe.session.user})
			...
			if len(missing_marsha) > 0:
				...
				frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "missing billing unit in marsha details for some employees", "file": cluser_file_upload["file"], "user": frappe.session.user})
		...
		frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "file is empty", "user": frappe.session.user})
