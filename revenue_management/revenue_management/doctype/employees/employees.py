...
def employees_import(file=None):
	try:
		if not file:
			frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "The file was not provided.", "user": frappe.session.user})
		...
		if len(excel_data_df) > 0:
			...
			if len(get_marsha_details) == 0:
				frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "Unable to locate Marsha Details.", "user": frappe.session.user})
			...
			if len(missing_marsha) > 0:
				...
				frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "Billing unit is absent in the Marsha details for certain employees.", "file": cluser_file_upload["file"], "user": frappe.session.user})
				return {"success": False, "message": "Billing unit is absent in the Marsha details for certain employees.", "file": cluser_file_upload["file"]}
		...
		frappe.publish_realtime("data_import_error", {"data_import": 'Employees',"show_message": "No data discovered in the HR file.", "user": frappe.session.user})
		return {"success": False, "message": "No data discovered in the HR file."}
...
def import_employees(file=None):
    try:
        ...
        return {"success": True, "Message": "Employees Import Initiates Shortly."}
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
