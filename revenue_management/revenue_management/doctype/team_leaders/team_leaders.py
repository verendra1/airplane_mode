...
def import_team_leader(file=None):
    try:
        ...
        if set(team_leader_details).issubset(excel_data_df.columns):
            get_employee_details = frappe.db.get_list("Employees", pluck="name")
            ...
            if len(get_team_leader_not_in_employee) > 0:
                ...
                get_team_leader_not_in_employee.to_csv(missing_employee_file_name, index=False)

                fileupload = upload_file_api(filename=missing_employee_file_name)
                if not fileupload["success"]:
                    return fileupload
                missing_employee_file = fileupload["file"]

            # TeamLeader exists in employee doctype conditon
            team_leaders_df = team_leaders_df[team_leaders_df["EID"].isin(get_employee_details)]
            ...
            if len(team_leaders_df) > 0:
                ...
                if len(remove_existing_team_leaders) > 0:
                    sort_new_team_leaders = remove_existing_team_leaders.sort_values(["Team Name"])
            ...
            return {"success": False, "message": "No data found", "missing_employees_file": missing_employee_file}
