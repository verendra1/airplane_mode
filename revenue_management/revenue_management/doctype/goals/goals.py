...
def goalsproductivity():
    ...
    quarter_wise = data['quarter_wise_data']
    print(quarter_wise,"#######################")

    quarter_df = pd.DataFrame.from_records(quarter_wise)
    quarter_df = quarter_df.rename(columns={"category":"CATEGORY","goal":"GOAL","productivity":"PRODUCTIVITY","achieved":"ACHIEVED","remaining":"REMAINING"})
    print(quarter_df,"//////////////////////")
   
    month_wise = [entry for sublist in data['month_wise_data'] for entry in sublist]
    print(month_wise,"<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>")
    month_df = pd.DataFrame.from_records(month_wise)
    month_df = month_df.rename(columns={"category":"CATEGORY","goal":"GOAL","productivity":"PRODUCTIVITY","achieved":"ACHIEVED","remaining":"REMAINING"})
    print(month_df,"*********************************   ")

    jan_df = month_df[month_df['month'] == 'Jan'].drop(columns=['month'])
    feb_df = month_df[month_df['month'] == 'Feb'].drop(columns=['month'])
    mar_df = month_df[month_df['month'] == 'Mar'].drop(columns=['month'])
    print(jan_df,"********************************************")
    print(feb_df,"********************************************")
    print(mar_df,"********************************************")
    print(month_df)

    dfs = [quarter_df, jan_df, feb_df, mar_df]
    excel_file_path = '/home/verendra/Downloads/goalsproductivity.xlsx'

    with pd.ExcelWriter(excel_file_path, engine='xlsxwriter') as writer:
        workbook = writer.book
        worksheet = workbook.add_worksheet('All_DataFrames')
        bold_format = workbook.add_format({'bold': True,'bg_color': '#F0FFFF','font_color': 'black'})
        worksheet.write(0, 0, "Property/Cluster Name:     ", bold_format)
        worksheet.write(1, 0, "Employee Name:     ", bold_format)
        worksheet.write(2, 0, "Employee ID:     ", bold_format)
        worksheet.write(3, 0, "Title:     ", bold_format)
        ...
        cell_format = workbook.add_format({'bold': True, 'bg_color': '#ADD8E6', 'border': 1, 'align': 'center'})
        for idx, df in enumerate(dfs):
            label = ['Q1', 'JAN', 'FEB', 'MAR'][idx]  # Label for each DataFrame
            worksheet.merge_range(start_row - 1, start_col, start_row - 1, start_col + len(df.columns) - 1, label, cell_format)
            df.to_excel(writer, sheet_name='All_DataFrames', startrow=start_row, startcol=start_col, index=False)
            header_format = workbook.add_format({'bold': True, 'bg_color': '#D4D4D4', 'border': 1})
            for col_idx, value in enumerate(df.columns.values):
                worksheet.write(start_row, start_col + col_idx, value, header_format)
